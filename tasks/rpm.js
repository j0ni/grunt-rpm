 ...
 * @returns {Array} list of file configuration filtered by invalid
 * sources and with additional configuration that may come along with
 * each file mapping.
 */
function filterFiles(grunt, files) {
  ...
  files.forEach(function (fileMapping) {
	...
	if (fileMapping.relativeTo && !grunt.file.isDir(fileMapping.relativeTo)) {
	  grunt.warn('The path specified for relativeTo is not a directory "%s"', fileMapping.relativeTo);
	}
	  ...
	  if (!grunt.file.exists(filepath)) {
		...
		if (fileMapping.directory) {
		  if (grunt.file.isDir(filepath)) {
			...
			grunt.log.warn(
              'The path %s is not a directory but it is marked as such',
              filepath,
              fileMapping.relativeTo);
		  }
		}
		...
		if (fileMapping.relativeTo) {
		  if (grunt.file.doesPathContain(fileMapping.relativeTo, filepath)) {
			...
			grunt.log.warn(
              'The path %s is not contained into the relativeTo directory %s',
              filepath,
              fileMapping.relativeTo);
		  }
		} else {
...
/**
 * Copy all the selected files to the tmp folder which wikll be the
 * buildroot directory for rpmbuild
 */
	  ...
	  try {
		...
		if (grunt.file.isDir(fileConfig.src)) {
		  if (fileConfig.directory) {
			...
			grunt.verbose.writeln('Copying folder "%s" to "%s"', fileConfig.src, + filepathDest);
			fs.copyRecursive(fileConfig.src, filepathDest, callback);
		  } else {
			// Create a folder inside the destination directory.
			grunt.verbose.writeln('Creating folder "%s"', filepathDest);
			grunt.file.mkdir(filepathDest);
			callback();
		  }
		} else {
		  // Copy a file to the destination directory inside the tmp folder.
		  grunt.verbose.writeln('Copying file "%s" to "%s"', fileConfig.src, filepathDest);
 ...
 * where rpmbuilder folder structure resides.
 * @param {Object} options - the options object.
 * @returns {Function} async function that executes the callback once
 * it is done.
 */
function spawnRpmbuild(grunt, buildPath, options) {
  return function (callback) {
	// FIXME make the hard-coded target architecture platform
